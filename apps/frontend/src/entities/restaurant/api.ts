...
export const UserRestaurantAPI = {
  ...
  post: async (data: UserRestaurant) => {
    ...
    if (!response.ok) {
      throw new Error(
        await getErrorMessage(response, 'Failed to create restaurant')
      );
    }
  },
  ...
  put: async (data: UserRestaurant) => {
    const response = await fetch(
      `${SERVER_URL}/user/restaurants/${data.user_restaurant_id}`,
      {
        method: 'PUT',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (!response.ok) {
      throw new Error(
        await getErrorMessage(response, 'Failed to update restaurant')
      );
    }
  },
  ...
  delete: async (data: UserRestaurant) => {
    ...
    if (!response.ok) {
      throw new Error(
        await getErrorMessage(response, 'Failed to delete restaurant')
      );
    }
  },
};

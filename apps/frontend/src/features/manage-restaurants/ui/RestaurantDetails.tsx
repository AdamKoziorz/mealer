    ...
    <div className="text-black p-16 flex flex-col gap-1">
      ...
      <TagList
        ...
        setLocalTags={(newTags) =>
          handleUpdate({ descriptors: newTags.map((t) => t.label) })
        }
      ...
      <TagList
        ...
        setLocalTags={(newTags) =>
          handleUpdate({ menu_items: newTags.map((t) => t.label) })
        }
      ...
      >
        <Button variant={'default'} onClick={handleBackClick} className="w-1/4">
          Close
        </Button>
        <Button
          variant={'secondary'}
